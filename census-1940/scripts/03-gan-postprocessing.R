...
# Define function to generate data from all four approaches in the paper
gen_synth_pums1940 <-
  function(dp = T,
           nmp = 1.5,
           mb = 500,
    ...
    if (dp) {
      model_path <-
        paste0('models/run', run, '/nmp-', nmp, '/pums1940-')
      
    } else {
      model_path <-
        paste0('models/run', run, '/nodp/pums1940-')
    }
    ...
    generator <- function(Z, temperature = temp) {
      ...
      G_age_inc_logits = G_log_prob[, 1:2]
      
      G_sex_logits = tf$concat(list(tf$zeros_like(G_log_prob[, 1, drop = F]), G_log_prob[, 3, drop = F]), 1L)
      ...
      G_race_logits = G_log_prob[, 16:21]
      ...
      G_hispan_logits = G_log_prob[, 22:26]
      ...
      G_marital_logits = G_log_prob[, 27:31]
    ...
    discriminator <- function(x) {
      ...
      D_h1_1 = tf$nn$relu(tf$matmul(D_h1, D_W1_1) + D_b1_1)
...
tmp_data_nodp <-
  lapply(synth_pums1940_nodp, function(run)
...
tmp_data_dp <-
  lapply(synth_pums1940_dp, function(run)
    lapply(run, function(x)
      lapply(x, function(y)
        synth_to_orig(y, gan_list))))
